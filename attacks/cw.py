...
class CW(Attack):
    ...
    def forward(self, images, labels):
        ...
        for step in range(self.steps):
            ...
            correct = correct.sum((1, 2)) / (pre.shape[1]*pre.shape[2])
            # print(correct.shape)
    ...
    def f(self, outputs, labels):
        ...
        one_hot_labels =   one_hot_labels.view(outputs.shape)
        ...
        if self._targeted:
            return torch.clamp((i-j).sum(), min=-self.kappa)
        else:
            return torch.clamp((j-i).sum(), min=-self.kappa)
