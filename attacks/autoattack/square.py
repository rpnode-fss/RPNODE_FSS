...
from auto_rog_libs.utilities.losses import Dice_metric
    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            ...
            for i_iter in range(self.n_queries):
                ...
                loss_min = min(loss, loss_min_curr)

                # print(x_new.shape, x_best.shape)
                improvement = self.dice(self.predict(x_best).detach(), y) - self.dice(self.predict(x_new).detach(), y) 
                if improvement>0:
                    x_new = x_new*improvement + (1-improvement)*x_best
                    x_best = x_new.squeeze(0).squeeze(0)
                n_queries[0] += 1.
